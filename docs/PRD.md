...
Единый Claude Code skill `/tgbot` с тремя подкомандами, который превращает деплой в диалог:

| Команда | Назначение | Частота использования |
|---------|-----------|----------------------|
| `/tgbot deploy` | Настройка сервера + деплой бота | При каждом новом боте |
| `/tgbot manage` | Статус, логи, редеплой, env, start/stop, delete | Повседневное управление |
| `/tgbot prepare` | Подготовка репо (Dockerfile, .env.example) | Одноразово перед первым деплоем |
| `/tgbot` | Интерактивное меню | Когда не помнишь подкоманду |
...
**Acceptance Criteria:**
- Пользователь вводит `/tgbot deploy https://github.com/user/bot`
...
**Acceptance Criteria:**
- `/tgbot manage` или `/tgbot manage status` показывает таблицу всех ботов
...
**Acceptance Criteria:**
- `/tgbot manage logs my-bot` показывает последние 100 строк логов
...
**Acceptance Criteria:**
- `/tgbot manage redeploy my-bot` обновляет код и перезапускает
...
**Acceptance Criteria:**
- `/tgbot manage env my-bot` показывает текущие переменные (маскируя секреты)
...
**Acceptance Criteria:**
- `/tgbot manage delete my-bot` с обязательным подтверждением
...
**Acceptance Criteria:**
- `/tgbot prepare https://github.com/user/bot` клонирует и анализирует
...
- Повторный запуск `/tgbot deploy` на тот же сервер не ломает существующее
