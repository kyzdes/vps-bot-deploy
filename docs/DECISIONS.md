...
## ADR-9: Единый скилл с подкомандами вместо трёх раздельных

**Статус:** Принято (v3, заменяет предыдущее решение)

**Контекст:** В v2 было три отдельных скилла: `/deploy-bot`, `/manage-bot`, `/prepare-bot`. Это работало, но создавало проблемы:
- Три скилла для одной задачи — пользователь должен помнить три имени
- Дублирование reference-файлов (dokploy-api.md копировался в deploy-bot и manage-bot)
- Установка: нужно копировать три папки

**Решение:** Единый скилл `/tgbot` с подкомандами: `deploy`, `manage`, `prepare`.

**Обоснование:**
- Один скилл — одна точка входа: `/tgbot`
- Подкоманды интуитивны: `/tgbot deploy`, `/tgbot manage logs my-bot`
- Без аргументов показывает интерактивное меню
- Reference-файлы не дублируются — один `dokploy-api.md` на все flows
- Установка: одна папка `skills/tgbot/`
...
**Жизненный цикл:**
```
/tgbot prepare  →  /tgbot deploy  →  /tgbot manage
